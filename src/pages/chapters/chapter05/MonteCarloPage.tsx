...
function MCBasicDemo() {
  ...
  const [result, setResult] = useState<
    | { type: 'single-eval'; q: number[][]; counts: number[][]; rmse: number; policy: Policy }
    | { type: 'policy-iteration'; iterations: MCBasicIteration[]; finalPolicy: Policy; finalQ: number[][]; finalQPolicy: Policy; rmse: number; stable: boolean }
    | null
  >(null);
  ...
  const displayPolicy = useMemo(() => {
    ...
    if (result.type === 'single-eval') {
      return showImproved ? greedyPolicy(result.q) : result.policy;
    }
  ...
  const displayValues = useMemo(() => {
    ...
    if (result.type === 'policy-iteration') {
      const qEst = result.iterations[iterIndex]?.qEstimate ?? result.finalQ;
      const pol = result.iterations[iterIndex]?.policyBefore ?? result.finalQPolicy;
  ...
  const lastIter = result?.type === 'policy-iteration' ? result.iterations[result.iterations.length-1] : null;
  const stableButMaxed = lastIter && !lastIter.policyStable && result?.type === 'policy-iteration' && result.iterations.length >= 20;

  return (
    <InteractiveDemo title="MC Basic：策略评估与策略迭代">
      <p className="text-xs text-gray-500 mb-2">
        本示例采用 continuing task。浏览器中使用长度为 T 的有限轨迹，以截断回报近似无限期折扣回报。T 越大截断偏差越小。
        {mode === 'single-eval' && ' RMSE 同时包含采样误差和有限轨迹截断误差。'}
      </p>
            ...
            {displayPolicy ? (
              <>
                ...
                {result?.type === 'single-eval' && (
                  <div className="mt-2 text-xs text-gray-500">{showImproved ? '贪心改进策略' : '被评估策略 π（随机策略）'}</div>
                )}
              </>
            ) : (
          ...
          {result?.type === 'single-eval' && (
            <div className="flex items-center gap-2 text-xs">
              <label className="flex items-center gap-1 cursor-pointer">
                <input type="checkbox" checked={showImproved} onChange={() => setShowImproved(!showImproved)} />
                显示贪心改进策略（根据 q_π 的一次改进 π'）
              </label>
            </div>
          )}

          {/* Three-panel iteration display */}
          {result?.type === 'policy-iteration' && (
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-800">策略迭代 —— 第 {iterIndex+1}/{result.iterations.length} 轮</h3>
                <div className="flex items-center gap-2">
                  <Button size="sm" variant="outline" disabled={iterIndex===0} onClick={()=>setIterIndex(v=>v-1)}>上一步</Button>
                  <Button size="sm" variant="outline" disabled={iterIndex>=result.iterations.length-1} onClick={()=>setIterIndex(v=>v+1)}>下一步</Button>
                </div>
              </div>
              <input type="range" min={0} max={result.iterations.length-1} value={iterIndex} onChange={e=>setIterIndex(Number(e.target.value))} className="w-full mb-3" />

              {currentIter && (
                <>
                  <div className="text-xs text-gray-600 mb-3">
                    π<sub>{currentIter.iteration}</sub> → MC 策略评估 → q<sub>π<sub>{currentIter.iteration}</sub></sub> → 贪心改进 → π<sub>{currentIter.iteration+1}</sub>
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    <div className="text-center">
                      <div className="text-[10px] font-medium text-gray-500 mb-1">π<sub>{currentIter.iteration}</sub> (评估前)</div>
                      <GridWorld config={config} policy={currentIter.policyBefore} compact className="max-w-full" />
                    </div>
                    <div className="text-center">
                      <div className="text-[10px] font-medium text-gray-500 mb-1">V<sub>π<sub>{currentIter.iteration}</sub></sub></div>
                      <GridWorld config={config} policy={currentIter.policyBefore}
                        values={policyWeightedStateValues(currentIter.qEstimate, currentIter.policyBefore)}
                        showValues compact className="max-w-full" />
                    </div>
                    <div className="text-center">
                      <div className="text-[10px] font-medium text-gray-500 mb-1">π<sub>{currentIter.iteration+1}</sub> (改进后)</div>
                      <GridWorld config={config} policy={currentIter.policyAfter} compact className="max-w-full" />
                    </div>
                  </div>
                  <div className="mt-3 text-xs text-gray-600 space-y-1">
                    <div>本轮有 <strong>{currentIter.changedStateCount}</strong> 个状态的动作发生变化</div>
                    {currentIter.policyStable
                      ? <div className="text-green-600 font-semibold">策略已稳定 ✓</div>
                      : <div className="text-amber-600">策略尚未稳定</div>}
                  </div>
                </>
              )}
            </div>
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <div className="flex justify-between text-sm text-gray-700 mb-1"><span>每对 (s,a) 采样回合数</span><span className="font-mono">{episodesPerPair}</span></div>
              ...
              <div>
                ...
                <Select value={String(horizonT)} onValueChange={(v)=>setHorizonT(Number(v))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {HORIZON_OPTIONS.map(t=><SelectItem key={t} value={String(t)}>T={t}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
          ...
          {result && (
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-base">统计</CardTitle></CardHeader>
                ...
                {result.type === 'single-eval' ? (
                  <>
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      真实值由模型计算仅用于验证；MC 训练本身不使用模型。RMSE 同时包含采样误差与有限轨迹截断误差。
                    </div>
                  </>
                ) : (
                  <>
                    <div>策略迭代轮数：<span className="font-mono font-semibold">{result.iterations.length}</span></div>
                    <div>策略是否稳定：<span className="font-mono">{result.stable ? '是 ✓' : (stableButMaxed ? '尚未观察到稳定' : '否')}</span></div>
                    <div>最终 RMSE(q_est, q_π)：<span className="font-mono">{result.rmse.toFixed(4)}</span></div>
                    <div className="text-xs text-gray-500 mt-1">真实值由模型计算仅用于验证。</div>
                    {stableButMaxed && <div className="text-xs text-amber-600 mt-1">当前有限样本和最大迭代轮数下，尚未观察到策略稳定。</div>}
                  </>
                )}
              </CardContent>
            </Card>
...
function MCExploringStartsDemo() {
  ...
  const runEpisodes = useCallback((n:number)=>{
    ...
    setLastTrajectory(mcBasic(config,1,horizonT).returns?.[0]?.[0] ? [] : []); // dummy — trajectory comes from run callback
    const traj = generateSampleTrajExploring(config,horizonT);
  ...
  return (<MCDemoShell title="MC Exploring Starts：随机初始状态-动作（增量训练）"
    ...
    chartData={rmseHistory} lastTrajectory={lastTrajectory}
...
function MCEpsilonGreedyDemo() {
  ...
  const runEpisodes = useCallback((n:number)=>{
    ...
    setLearnerState(ns);
    setRmseHistory(prev=>[...prev,{episode:ns.episodesCompleted,rmse:qTableRMSE(ns.q,qStar)}]);
    const traj = mcEpsilonGreedy(config,1,horizonT,ns.currentEpsilon,visitMode,schedule);
  ...
  return (<MCDemoShell title="MC ε-Greedy：持续探索的模型-free 控制（增量训练）"
    ...
    currentEpsilon={effectiveEpsilon}
    horizonT={horizonT} onHorizonTChange={setHorizonT}
...
interface MCDemoShellProps {
  ...
  horizonT?: number;
  onHorizonTChange?: (v: number) => void;
  rmseNote?: string;
}
...
function MCDemoShell(props: MCDemoShellProps) {
  ...
  return (
    <InteractiveDemo title={title}>
      <p className="text-xs text-gray-500 mb-2">
        本示例为 continuing task。为便于浏览器计算，使用长度为 T 的有限轨迹，以截断回报近似无限期折扣回报。T 越大截断偏差越小。
      </p>
      <div className="grid lg:grid-cols-[1fr_340px] gap-6">
        <div className="flex flex-col gap-4">
          <div className="flex flex-col items-center justify-center bg-gray-50 rounded-xl p-6 border border-gray-200">
          ...
          <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
            <LineChart data={chartData} xKey="episode" xLabel="回合数" yLabel="RMSE(q, q*)"
              series={[{ key: 'rmse', name: 'RMSE', color: '#2563eb' }]} height={200} />
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              <div className="overflow-x-auto"><table className="w-full text-xs text-left">
                ...
                <tbody className="text-gray-700">
                  {lastTrajectory.slice(0,12).map((step,idx)=>(<tr key={idx} className="border-b last:border-0">
                    <td className="py-1">{idx}</td><td>s{step.state+1}</td><td>{ACTION_NAMES[step.action]}</td><td>{step.reward.toFixed(2)}</td></tr>))}
                </tbody>
              </table></div>
            </div>
          )}
        ...
        <div className="space-y-4">
          ...
          <Card><CardHeader className="pb-2"><CardTitle className="text-base">更新方式</CardTitle></CardHeader>
            <CardContent>
              <Select value={visitMode} onValueChange={(v)=>onVisitModeChange(v as VisitMode)}>
                <SelectTrigger><SelectValue placeholder="选择 visit 模式" /></SelectTrigger>
                <SelectContent><SelectItem value="first-visit">First-visit</SelectItem><SelectItem value="every-visit">Every-visit</SelectItem></SelectContent>
              </Select>
            </CardContent>
          </Card>
          {onEpsilonChange!==undefined && epsilon!==undefined && (
            <Card><CardHeader className="pb-2"><CardTitle className="text-base">探索参数 <KaTeX math={String.raw`\varepsilon`} /></CardTitle></CardHeader>
              <CardContent className="space-y-3">
                ...
                {onScheduleChange && schedule!==undefined && (
                  <div><label className="text-xs text-gray-600 block mb-1">ε 调度方式</label>
          ...
          {onHorizonTChange!==undefined && horizonT!==undefined && (
            <Card><CardHeader className="pb-2"><CardTitle className="text-base">轨迹长度 T</CardTitle></CardHeader>
              <CardContent>
                <Select value={String(horizonT)} onValueChange={(v)=>onHorizonTChange(Number(v))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{HORIZON_OPTIONS.map(t=><SelectItem key={t} value={String(t)}>T={t}</SelectItem>)}</SelectContent>
                </Select>
              </CardContent>
            </Card>
          )}
